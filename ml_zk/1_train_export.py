...
class EligibilityModel(nn.Module):
    ...
    def forward(self, x):
        # We REMOVE Sigmoid from the ONNX graph to drastically reduce ZK circuit complexity
        # (avoiding expensive non-linear lookup tables).
        # We just output the raw logit. The threshold check (logit > 0) is equivalent to Sigmoid(logit) > 0.5.
...
def main():
    ...
    # 2. Train with BCEWithLogitsLoss + Adam optimizer.
    ...
    with torch.no_grad():
        preds = (model(X_tensor) > 0.0).float()
